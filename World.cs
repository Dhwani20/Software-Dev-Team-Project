...
{
    //https:/maps.googleapis.com/maps/api/geocode/json?latlng=55.4018502,10.3419367&zoom=18&size=512x512&scale=2&maptype=roadmap&format=xml;
    ...
    void Update()
    {
        if (worldGrid.needsUpdate)
        {
            ...
            for (int index = 0; index < itemsToRender.Count; index++)
            {
                ...
                StartCoroutine(itemsToRender[index].Download());
            }
